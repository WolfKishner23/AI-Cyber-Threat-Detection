...
function Dashboard() {
  ...
  const filteredAlerts = useMemo(() => {
    return alerts.filter(a => {
      const customerId = a.event?.user_id || '';
      const fullName = customerMap[customerId]?.full_name || '';
      
      const matchesSearch = filters.search === '' || 
                            a.alert_type.toLowerCase().includes(filters.search.toLowerCase()) ||
                            a.id.toString().includes(filters.search) ||
                            customerId.toLowerCase().includes(filters.search.toLowerCase()) ||
  ...
  }, [alerts, filters, customerMap]);

  const filteredInvestigations = useMemo(() => {
    return investigations.filter(i => {
      const fullName = customerMap[i.customer_id]?.full_name || '';
  ...
  }, [investigations, filters, customerMap]);
                    ...
                    <tbody>
                      {events.slice(0, 10).map(evt => (
                        <tr key={evt.id}>
                          ...
                          <td>
                            <span className="badge status-new">{evt.event_type}</span>
                            {evt.raw_payload?.simulation?.issue_summary && (
                              <div style={{ marginTop: '0.25rem', fontSize: '0.75rem', color: 'var(--risk-high)', fontStyle: 'italic' }}>
                                ⚠️ {evt.raw_payload.simulation.issue_summary}
                              </div>
                            )}
                          </td>
